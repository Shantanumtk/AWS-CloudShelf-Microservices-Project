...
print_info "Building Backend artifacts..."
...
docker build -t stock-check-service:latest stock-check-service/
...
print_info "Setting up port forwarding for Ingress..."
pkill -f "kubectl port-forward.*ingress-nginx-controller.*$INGRESS_PORT" 2>/dev/null || true
sleep 2
...
PF_PID=$!
echo $PF_PID > /tmp/ingress-forward.pid
sleep 3

if ps -p $PF_PID > /dev/null; then
    print_success "Port forwarding started on port $INGRESS_PORT (PID: $PF_PID)"
else
    print_error "Port forwarding may have failed. Check /tmp/ingress-forward.log"
fi
